  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`/api/balpoom?local_id=1`);
...
//----------------------------------------------------------------------------------------------------
  ...
  // showmap.js 수정
  const filteredPositions = Array.isArray(dopositions) // 1. 배열인지 확인하고
    ? dopositions.filter(position => 
      position.content && position.content.includes(search) // 2. content 존재 확인
      )
  : []; // 3. 배열이 아니면 빈 배열로 대체
